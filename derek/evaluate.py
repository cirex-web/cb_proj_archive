...
def get_scores():
  ...
  # read in files 
  print("Starting to get Scores")
  for path,_,files in os.walk("simulatedmRNAs"):

    if path == "simulatedmRNAs":
        continue

    transcript_id = path.replace("simulatedmRNAs/","")
...
### VISUALIZING ###
transcript_to_score_map = get_scores()

for entry in transcript_to_score_map.values():
  print(entry.keys())
...
sns.boxplot(data=data,linewidth=1).set(title="GMAP and Pairagon extended comparison on simulated mouse cDNA (2nt ext)")
